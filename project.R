#@title Loading data from copy/paste, don't ask why!  Just run the cell... { display-mode: "form" }
...
library(ggplot2)
boxplot(scale(country.data))


countries2 <- country.data

rownames(countries2) <- country.names
...
# (usually for small number of clusters)

# Dissimilarity matrix
d <- dist(countries2.scaled, method = "euclidean")^2
d**2
...
# Compute with agnes
hc2 <- agnes(countries2.scaled, method = "complete")

# Agglomerative coefficient
hc2$ac
# 0.9130134 (values closer to 1 suggest strong clustering structure)
...
# As we move up the tree, observations that are similar to each other are combined
# into branches, which are themselves fused at a higher height.
...
# the less similar the observations are. Note that, conclusions about the proximity
# of two observations can be drawn only based on the height where branches
# containing those two observations first are fused. We cannot use the proximity
# of two observations along the horizontal axis as a criteria of their similarity.
...
# ----- Identify sub-groups (clusters) by cutting the dendogram -----
...
# add a border around the 4 clusters
jpeg('dendrogram_ward_d2.jpg')
plot(hc5, cex = 0.6)
rect.hclust(hc5, k = 4, border = 2:5)
cat(dev.off())

# Cut agnes() tree into 4 groups
hc_a <- agnes(countries2.scaled, method = "ward")
plot(hc_a, cex = 0.6)
rect.hclust(hc_a, k = 4, border = 2:5)
cutree(as.hclust(hc_a), k = 4)

# Cut diana() tree into 4 groups
hc_d <- diana(countries2.scaled)
cutree(as.hclust(hc_d), k = 4)


# compare the two dendrograms
# Compute distance matrix
res.dist <- dist(countries2.scaled, method = "euclidean")
...
set.seed(123)
country.scaled <- scale(country.data)
k4 <- kmeans(country.scaled, 4, nstart = 20)
plot(country.data, col = k4$cluster, pch=20)
country.names[k4$cluster == 2]

plot(mtcars$cyl, mtcars$mpg, col=2)
...
p3 <- fviz_cluster(k4, axes = c(1,2,3), geom = "point",  data = country.data) + ggtitle("k = 4")
...
p1 <- fviz_cluster(k4, axes = c(1,2), geom = "point",  data = country.data)+ ggtitle("PC1 vs PC2")
p2 <- fviz_cluster(k4, axes = c(1,3), geom = "point",  data = country.data)+ ggtitle("PC1 vs PC3")
p3 <- fviz_cluster(k4, axes = c(1,4), geom = "point",  data = country.data)+ ggtitle("PC1 vs PC4")
p4 <- fviz_cluster(k4, axes = c(2,3), geom = "point",  data = country.data)+ ggtitle("PC2 vs PC3")
p5 <- fviz_cluster(k4, axes = c(2,4), geom = "point",  data = country.data)+ ggtitle("PC2 vs PC4")
p6 <- fviz_cluster(k4, axes = c(3,4), geom = "point",  data = country.data)+ ggtitle("PC3 vs PC4")
cowplot::plot_grid(p1,p2,p3,p4,p5,p6, ncol=3)

library(dplyr)
c.pc <- prcomp(country.data, scale=TRUE)
summary(c.pc)
c.pc
countries.pc1 <- as.data.frame(cbind(country.names,c.pc$x[,1]))
colnames(countries.pc1) <- c("country", "pc")
countries.pc1$pc <- as.numeric(countries.pc1$pc)
countries.pc1 <- arrange(countries.pc1, pc)
head(countries.pc1)
tail(countries.pc1)

countries.pc2 <- as.data.frame(cbind(country.names,c.pc$x[,2]))
colnames(countries.pc2) <- c("country", "pc")
countries.pc2$pc <- as.numeric(countries.pc2$pc)
countries.pc2 <- arrange(countries.pc2, pc)
head(countries.pc2)
tail(countries.pc2)

countries.pc3 <- as.data.frame(cbind(country.names,c.pc$x[,3]))
colnames(countries.pc3) <- c("country", "pc")
countries.pc3$pc <- as.numeric(countries.pc3$pc)
countries.pc3 <- arrange(countries.pc3, pc)
head(countries.pc3)
tail(countries.pc3)

countries.pc4 <- as.data.frame(cbind(country.names,c.pc$x[,4]))
colnames(countries.pc4) <- c("country", "pc")
countries.pc4$pc <- as.numeric(countries.pc4$pc)
countries.pc4 <- arrange(countries.pc4, pc)
head(countries.pc4)
tail(countries.pc4)
...
p1 <- ggplot() + geom_point(aes(c.pc$x[,1], c.pc$x[,2]))
p2 <- ggplot() + geom_point(aes(c.pc$x[,1], c.pc$x[,3]))
p3 <- ggplot() + geom_point(aes(c.pc$x[,2], c.pc$x[,3]))
p4 <- ggplot() + geom_point(aes(c(1,2,3,4,5,6,7,8,9), c.pc$sdev))
cowplot::plot_grid(p1,p2,p3,p4, ncol=2)

geom_point(aes(iris.pc$x[,1], iris.pc$x[,2], color=iris$Species)) +
...
set.seed(123)
country.data
country.data.scaled <- scale(country.data)

k4 <- kmeans(country.data.scaled, 4, nstart = 25)

hc.a <- agnes(country.data.scaled, method = "ward")
pltree(hc.a, cex=0.6, hang=-1, main="agnes")
rect.hclust(as.hclust(hc.a), k=4)
agnescluster <- cutree(as.hclust(hc.a), k = 4)

hc.d <- diana(country.data.scaled)
pltree(hc.d, cex=0.6, hang=-1, main="diana")
rect.hclust(as.hclust(hc.d), k=4)
dianacluster <- cutree(as.hclust(hc.d), k=4)

country <- cbind(country.names, country.data)
country$clu.kmeans <- k4$cluster
country$clu.agnes <- agnescluster
country$clu.diana <- dianacluster
country[which(country$clu.diana == 1),1]


library(plyr)
country$clu.diana <- revalue(as.factor(country$clu.diana), c("2"="1", "1"="2", "4"="3", "3"="4"))
country$clu.diana <- relevel(as.factor(country$clu.diana), "4")
country$clu.diana <- factor(country$clu.diana, levels=rev(levels(country$clu.diana)))
levels(as.factor(country$clu.diana))

p1<-ggplot(country, aes(x=factor(clu.diana), y=child_mort)) +
    geom_boxplot() + labs(x='');p1

# Cluster 3 and 4 have very high rates of child mortality in comparison of cluster 2 and 1.

p2<-ggplot(country, aes(x=factor(clu.diana), y=exports)) +
    geom_boxplot() + labs(x='')

# cluster 1 has significantly higher exports compared to the rest

p3<-ggplot(country, aes(x=factor(clu.diana), y=imports)) +
    geom_boxplot() + labs(x='')

# cluster 1 has much higher imports.  cluster 4 (nigeria) has much smaller exports 
# but that's not out of the realm of an "adequate" country. 

p4<-ggplot(country, aes(x=factor(clu.diana), y=health)) +
    geom_boxplot() + labs(x='')

# cluster 1 and 2 have higher health spending.  cluster 3 has a wide range of 
# health spending

p5<-ggplot(country, aes(x=factor(clu.diana), y=income)) +
    geom_boxplot() + labs(x='')

# cluster 3 and 4 both have very low incomes while cluster 1 has very high incomes

p6<-ggplot(country, aes(x=factor(clu.diana), y=inflation)) +
    geom_boxplot() + labs(x='')

# cluster 4 has extreme levels of inflation while the rest seem reletively the same
# There is a trend of higher levels of inflation from cluster 1 to cluster 4

p7<-ggplot(country, aes(x=factor(clu.diana), y=life_expec)) +
    geom_boxplot() + labs(x='')

# cluster 3 and 4 have much lower life expectancy

p8<-ggplot(country, aes(x=factor(clu.diana), y=total_fer)) +
    geom_boxplot() + labs(x='')

# cluster 3 and 4 have very high rates of fertility (75% of data is greater than the rest)

p9<-ggplot(country, aes(x=factor(clu.diana), y=gdpp)) +
    geom_boxplot() + labs(x='')
# cluster 3 and 4 have very very low GDPs (almost all the data is lower than 75% 
# and 100% of cluster 2 and 1 respectively)

cowplot::plot_grid(p1,p2,p3,p5,p6,
                   p7,p8,p9,
                   ncol=4)
...
p1<-ggplot(country, aes(x=factor(clu.agnes), y=child_mort)) +
    geom_boxplot() + labs(x='');p1

p2<-ggplot(country, aes(x=factor(clu.agnes), y=exports)) +
    geom_boxplot() + labs(x='')

p3<-ggplot(country, aes(x=factor(clu.agnes), y=imports)) +
    geom_boxplot() + labs(x='')

p4<-ggplot(country, aes(x=factor(clu.agnes), y=health)) +
    geom_boxplot() + labs(x='')

p5<-ggplot(country, aes(x=factor(clu.agnes), y=income)) +
    geom_boxplot() + labs(x='')

p6<-ggplot(country, aes(x=factor(clu.agnes), y=inflation)) +
    geom_boxplot() + labs(x='')

p7<-ggplot(country, aes(x=factor(clu.agnes), y=life_expec)) +
    geom_boxplot() + labs(x='')

p8<-ggplot(country, aes(x=factor(clu.agnes), y=total_fer)) +
    geom_boxplot() + labs(x='')

p9<-ggplot(country, aes(x=factor(clu.agnes), y=gdpp)) +
    geom_boxplot() + labs(x='')
